...
from guillotina_s3storage.storage import S3Manager
...
async def test_save_same_chunk_multiple_times(own_dummy_request, mock_txn):
    ...
    with own_dummy_request:
        ...
        async with S3Manager(util.opts) as s3client:
            ...
            mpu = await s3client.create_multipart_upload(
                Bucket=bucket_name, Key=upload_file_id
            )

            part = await s3client.upload_part(
                Bucket=bucket_name,
                Key=upload_file_id,
                PartNumber=block,
            ...
            ob = await s3client.get_object(Bucket=bucket_name, Key=upload_file_id)
            data = b""
...
async def _test_exc_backoff(util):
    async with S3Manager(util.opts) as s3client:
        error_class = s3client.exceptions.from_code("InvalidPart")
    raise error_class(
        {
            "Error": {
                "Code": "InvalidPart",
